...
import { INITIAL_RECORDS } from "./src/data";
...
// POST AI analyze chief complaint
app.post("/api/ai/analyze", async (req, res) => {
  try {
    ...
    const ai = getGemini();
    const prompt = `วิเคราะห์อาการสำคัญของผู้ป่วยดังนี้: "${chiefComplaint}"`;

    const response = await ai.models.generateContent({
      model: "gemini-3.5-flash",
      contents: prompt,
      config: {
        systemInstruction: `คุณเป็นผู้ช่วยวิชาชีพนักกายภาพบำบัดผู้เชี่ยวชาญ คอยวิเคราะห์อาการสำคัญ (Chief Complaint) ของผู้ป่วย แล้วคาดเดาวินิจฉัยโรคทางกายภาพบำบัด (PT Diagnosis) และแนะนำแผนการรักษาที่เหมาะสม (Treatments), ค่าบริการ และคำแนะนำสั้นๆ
ให้ตอบกลับในรูปแบบ JSON ตาม Schema ที่กำหนดนี้เสมอ โดยห้ามแต่งตัวแปรนอกเหนือจากนี้:
- "ptDiagnosis": ข้อเสนอแนะการวินิจฉัยโรคทางกายภาพบำบัด (เช่น MPS, Frozen Shoulder, OA Knee, LBP, C-Spondylosis, Spinal Stenosis, Piriformis Syndrome, Herniated Disc, De Quervain, Plantar Fasciitis, Tennis Elbow, Bell's Palsy เป็นต้น)
- "suggestedTreatments": อาร์เรย์ของวิธีการรักษากายภาพบำบัดที่แนะนำ (เลือกจากตัวย่อทางการแพทย์: "US", "PMS", "Hot Pack", "Traction", "Exercise", "Laser", "ES", "Manual Therapy")
- "suggestedFee": ตัวเลขค่าธรรมเนียมการรักษาที่แนะนำในสกุลเงินบาท (ตัวเลือกทั่วไป เช่น 450, 500, 800, 1000)
- "remarks": บันทึกผลการวิเคราะห์ คำอธิบาย หรือคำแนะนำเพิ่มเติมที่กระชับ`,
        ...
        responseSchema: {
          ...
          properties: {
            ptDiagnosis: {
              type: Type.STRING,
              description: "The suggested physical therapy diagnosis."
            },
            suggestedTreatments: {
              type: Type.ARRAY,
              items: { type: Type.STRING },
              description: "Array of recommended treatment IDs (must match options)."
            },
            suggestedFee: {
              type: Type.INTEGER,
              description: "Suggested service fee in Baht."
            },
            remarks: {
              type: Type.STRING,
              description: "Additional analysis and recommendations."
            }
          },
          required: ["ptDiagnosis", "suggestedTreatments", "suggestedFee", "remarks"]
        }
      }
    });
